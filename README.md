...
![Hooks](https://img.shields.io/badge/hooks-15-blue)
...
Each agent has a defined role, knows its limits, and hands work off to the next agent in the chain. Domain knowledge lives in 26 on-demand skills. Quality invariants are enforced by 15 platform-level hooks that the model cannot override.
...
| **Instructions** (`.instructions.md`)                 | 9     | Global rules auto-applied to every session (core behavior, Python, SQL, YAML, PowerShell, TypeScript, Markdown, JSON, Git commit standards)                          |
| **Hooks** (`.ps1`)                                    | 15    | Platform-level enforcement: quality gates, secrets scan, destructive command blocking, etc.                                   |
...
Verify: open Copilot Chat → Command Palette → "Chat: Configure Hooks". You should see 15 lifecycle hooks registered (all in `hooks.json`, including `verify-hook-integrity.ps1` as a SessionStart hook).
