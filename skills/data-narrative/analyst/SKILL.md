...
- **Load `data-science` skill first.** Follow its EDA Workflow for profiling: shape, types, missingness, distributions, outliers, correlations, and class balance. The `data-science` skill is the authoritative reference for statistical testing, modeling, and experiment analysis.
...
- `"relationship": "join"` → merge on `join_key` before profiling (`pd.merge`). Profile both the individual files and the joined result. Cross-file findings (e.g., "customers in region X have 2× higher order value") are the most valuable angle for the Editor.
- Never profile files in isolation when a relationship is declared — the joined dataset is the unit of analysis.
...
Load `detective.json` to read domain context before touching the data. This prevents misinterpreting column names or units (e.g., knowing "revenue" is in thousands, not raw dollars).
...
For each analysis area that yields a non-trivial result, write a Python script to `narrative-output/analyst/`. Name scripts descriptively:
