...
Read all `high` significance findings from `analyst.json`. Ask:
...
- Section `##` headers only — no `###` subsections unless a supporting evidence section genuinely needs them
- Every specific statistic must appear verbatim as computed in `analyst.json` — do not rephrase numbers
- Use `>` blockquotes to call out the single most important number in each section
- Maximum 800 words for the body (excluding Provenance) — the value is in precision, not length
