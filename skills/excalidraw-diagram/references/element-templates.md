...
```json
{
  ...
  "strokeColor": "<Start/Trigger stroke from palette>",
  "backgroundColor": "<Start/Trigger fill from palette>",
  "fillStyle": "solid",
  "strokeWidth": 2,
  "strokeStyle": "solid",
  "roughness": 0,
  ...
  "isDeleted": false,
  "groupIds": [],
  "boundElements": [{"id": "start_text1", "type": "text"}],
  "link": null,
  "locked": false
}
```
...
For curved arrows: use 3+ points in the `points` array (e.g., `[[0, 0], [60, -40], [120, 0]]`).
...
**When an arrow connects to a shape:**

1. The arrow's `startBinding` or `endBinding` must reference the shape's `id`.
2. The shape's `boundElements` array must include the arrow's `id`: `{"id": "arrow1", "type": "arrow"}`.
3. Both sides must be consistent - if the arrow references the shape, the shape must reference the arrow.

**When text is inside a shape:**

1. The text element's `containerId` must reference the shape's `id`.
2. The shape's `boundElements` array must include the text's `id`: `{"id": "text1", "type": "text"}`.
3. Set text `textAlign: "center"` and `verticalAlign: "middle"` for centered text.
